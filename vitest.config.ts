...
export default defineConfig({
  test: {
    ...
    // scan-service's card-fingerprint path reads process.env.CARD_FINGERPRINT_SECRET (and throws
    // if unset). The value here is test-only and must never be a real production secret.
    env: {
      CARD_FINGERPRINT_SECRET: 'test-secret-do-not-use-in-prod',
    },
