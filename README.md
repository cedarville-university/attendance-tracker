...
| `LTI_TOOL_SIGNING_KEYS_JSON` | no | unset -> a key is generated on first boot and **persisted** to the `tool_signing_keys` table (stable across restarts) | JSON array of `{ kid, privateKeyPkcs8Pem, status: 'active' \| 'previous' }` for this app's own RSA signing keys, published at `GET /lti/jwks`. When set it takes precedence over the DB table. Production may set this so keys live in a secret store; otherwise rotate via the admin setup page (`/admin.html`). MUST NOT be committed to Git. |
| `SETUP_TOKEN` | no | unset -> the admin page's token bootstrap is disabled | Bootstrap credential (≥ 16 chars) for `/admin.html`: lets you stand up the first Canvas connection before an LTI Administrator-role launch exists. Sent as the `x-setup-token` header. Leave unset once an admin launch works. MUST NOT be committed to Git. |
