...
Standing up a custom domain is a **ladder**, not a switch, because each rung needs
the previous one to already exist:

1. A managed certificate is validated **against live DNS at provisioning time**, so
   DNS must resolve first. But the DNS record points at the Container App's FQDN,
   which does not exist until the app does.
2. Azure refuses to create a managed certificate at all unless the hostname is
   **already registered** on a container app in the environment
   (`RequireCustomHostnameInEnvironment`) — but a `customDomains` entry that carries
   a `certificateId` cannot be written before that certificate exists.

So the hostname has to be registered with its binding **disabled** first, and the
certificate issued and bound in a *separate* deployment.

`customDomainMode` (`none` | `hostname` | `bound`, default `bound`) expresses the
rungs:

| Pass | Command | Result |
| --- | --- | --- |
| 1 | `-p customDomainMode=none` | foundation provisions; **web + worker fail** on unresolvable Key Vault secret refs — expected |
...
| 3 | `-p customDomainMode=hostname` | hostname registered, binding `Disabled`; Azure validates it against DNS |
| 4 | deploy **without** the flag (`bound`) | certificate issues and the binding switches to `SniEnabled` |
...
CI never passes `customDomainMode`, so pipeline deploys always run at `bound`. That
is correct in steady state — the ladder has been climbed, the certificate exists,
and the deploy is a no-op on all of it. **Only a from-scratch environment needs
passes 3 and 4 as separate steps**; skipping straight to `bound` fails with
`RequireCustomHostnameInEnvironment`.

> **Never bind the custom domain by hand.** Adding the hostname through the portal
> or `az containerapp hostname add` creates a managed certificate under an
> auto-generated, timestamped name (`attendance.cedarville.edu-cae-atte-260903182113`).
> Container Apps then refuses to create a second managed certificate for the same
> subject, so **every subsequent Bicep deploy fails** with
> `DuplicateManagedCertificateInEnvironment` — which means every release fails, not
> just the next one. This happened on the prod bootstrap. Recovery is to unbind and
> delete, then let Bicep create the certificate under its own deterministic name:
>
> ```bash
> az containerapp hostname delete -g rg-attendance-<ENV> -n ca-attendance-<ENV>-web \
>   --hostname <hostname> --yes
> az containerapp env certificate delete -g rg-attendance-<ENV> -n cae-attendance-<ENV> \
>   --certificate <auto-generated-name> --yes
> # then re-run the pass-3 deploy
> ```
>
> The certificate must be deleted *after* the hostname is unbound — a certificate in
> use cannot be removed. Do this while the app is still on the placeholder image and
...
- The four-pass sequence above, because prod is the first environment whose custom
  domain was stood up entirely through Bicep.
