...
Insights, and a Key Vault. The compute resources (Postgres Flexible Server,
Container Apps environment, web app, worker, migrate job) are appended by a later
task and deployed by CI/CD after that.
...
# 1. Session cookie signing secret (random 32+ bytes, base64).
az keyvault secret set --vault-name "$KV" --name app-session-secret \
  --value "$(openssl rand -base64 48)"

# 2. LTI tool signing keys as a JSON array of JWKs (generated by the tool's keygen).
az keyvault secret set --vault-name "$KV" --name lti-tool-signing-keys-json \
  --file ./lti-tool-signing-keys.json

# 3. Card-fingerprint HMAC key (random 32+ bytes, base64).
az keyvault secret set --vault-name "$KV" --name card-fingerprint-secret \
  --value "$(openssl rand -base64 32)"

# 4. Identity/roster resolver API key (issued by the identity service).
az keyvault secret set --vault-name "$KV" --name identity-api-key \
  --value "<identity-service-api-key>"

# 5. Postgres connection string. Migrations live in server/migrations and are
...
# 6. Application Insights connection string (deployment output
